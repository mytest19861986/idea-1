...
DISPATCH_PROVENANCE_REPORT = """سلام فرمانده، آنتی‌گرویتی هستم.

پیرو گزارش اولیه پیش‌پرواز، متن کامل و شواهد خام دریافتی از ارزیابی ریلیز توسط مدل‌های تخصصی (Gemini Cloud و Qwen) به همراه گزارش نهایی تقدیم می‌گردد:

==================================================
1. RAW DOMAIN AUDIT: GEMINI CLOUD (Domain Expert)
==================================================
```plaintext
...
- UNKNOWN semantics preserved: Missing factors, confidence states, timestamps (observedAt), and freshness metrics default to null or explicit "UNKNOWN" states without midpoint fabrication.
- Scoring and confidence are decoupled under scoringModelVersion: V1_BALANCED.
- All 12 enumerated monetization models are accounted for in schema validation.
- Data isolation verified: REAL_SOURCE_DATA_FLOWING: NO and PRODUCTION_TOUCHED: NO.
- Calibration & Scope: Claims are bounded strictly to restricted production preflight. No claims of high availability, high scale, multi-region deployment, or unrestricted external data ingress.
```

==================================================
2. STRUCTURED PREFLIGHT REPORT (PRODUCT-RELEASE-001)
==================================================
```plaintext
...
لطفاً حکم و تعیین تکلیف رسمی خود در خصوص این ریلیز کاندید را اعلام بفرمایید."""
...
async def main():
    ...
    async with websockets.connect(ws_url, max_size=50*1024*1024) as ws:
        ...
        (() => {{
            ...
            document.execCommand('insertText', false, {json.dumps(DISPATCH_PROVENANCE_REPORT)});
        ...
        await ws.recv()
        await ws.recv()
        print("DISPATCH_PROVENANCE_REPORT delivered.")
