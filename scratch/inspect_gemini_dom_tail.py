...
async def main():
    ...
    async with websockets.connect(ws_url, max_size=50*1024*1024) as ws:
        js = """
        (() => {
            const isGenerating = !!document.querySelector('.loading-dots, .streaming, mat-progress-bar, [aria-label*="Stop" i]');
            const text = document.body ? document.body.innerText.slice(-1500) : '';
            return { isGenerating, tail: text };
        })()
        """
