...
async def direct_type_and_click(tab_id, name, text):
    ...
    async with websockets.connect(ws_url, max_size=50*1024*1024) as ws:
        ...
        js = f"""
        (() => {{
            const input = document.querySelector('textarea, rich-textarea, div[contenteditable="true"], .chat-input, #prompt-textarea');
            if (input) {{
                input.focus();
                if (input.tagName === 'TEXTAREA' || input.tagName === 'INPUT') {{
                    input.value = {json.dumps(text)};
                }} else {{
                    input.innerText = {json.dumps(text)};
                }}
                input.dispatchEvent(new Event('input', {{ bubbles: true }}));
                input.dispatchEvent(new Event('change', {{ bubbles: true }}));
                
                setTimeout(() => {{
                    const btn = document.querySelector('button[aria-label*="Send" i], button[data-testid="send-button"], button.send-btn, button.mat-mdc-icon-button, button.ant-btn-primary') ||
        ...
        raw2 = await ws.recv()
        print(f"[{name}] DIRECT INSERT & CLICK:", json.loads(raw2).get('result', {}).get('result', {}).get('value', {}))
        
        await asyncio.sleep(1.0)
        await ws.send(json.dumps({"id": 3, "method": "Input.dispatchKeyEvent", "params": {"type": "rawKeyDown", "windowsVirtualKeyCode": 13, "text": "\r", "key": "Enter", "code": "Enter"}}))
        await ws.send(json.dumps({"id": 4, "method": "Input.dispatchKeyEvent", "params": {"type": "keyUp", "windowsVirtualKeyCode": 13, "key": "Enter", "code": "Enter"}}))
        await ws.recv()
        await ws.recv()

async def main():
    await direct_type_and_click(QWEN_TAB_ID, "Qwen", PROMPT_QWEN)
    await asyncio.sleep(1.0)
    await direct_type_and_click(GEMINI_TAB_ID, "Gemini", PROMPT_GEMINI)
    await asyncio.sleep(1.0)
    await direct_type_and_click(GLM_TAB_ID, "GLM", PROMPT_GLM)
