...
async def main():
    ...
    async with websockets.connect(ws_url, max_size=50*1024*1024) as ws:
        ...
        # Focus & Type directly via Input events and DOM manipulation
        js = f"""
        (() => {{
            ...
            if (el) {{
                ...
                return {{ ok: true, tag: el.tagName, len: el.innerText.length }};
            }}
            return {{ ok: false }};
        }})()
        """
        ...
        raw2 = await ws.recv()
        print("GEMINI INSERTION:", json.loads(raw2).get('result', {}).get('result', {}).get('value', {}))
